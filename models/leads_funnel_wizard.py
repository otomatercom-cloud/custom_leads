...
class LeadsFunnelWizard(models.TransientModel):
    ...
    # Related fields to control visibility
    ...
    def action_whatsapp(self):
        self.ensure_one()
        # Call the existing method on the lead if available or implemented here
        # The user requested "whatsapp", "second follow up", etc.
        # Assuming we use the existing action_send_whatsapp_intro or similar logic
        # But for now, just marking the boolean as per request logic
        
        # Checking if there is an existing method for whatsapp in leads.py
        # There is 'action_send_whatsapp_intro' in leads.py
        self.lead_id.action_send_whatsapp_intro()
        # The above method might return an action, but we want to stay in wizard if possible or reload it.
        # action_send_whatsapp_intro returns a 'reload' client action.
        
        # We need to manually set the fields if the method doesn't do what we want entirely for this flow
        # The method sets whatsapp_intro = True.
        return self.reload_wizard()

    def action_second_followup(self):
        self.ensure_one()
        self.lead_id.write({
            'second_followup': True,
            'second_followup_dt': fields.Datetime.now()
        })
    ...
    def action_course_wise_webinar(self):
        self.ensure_one()
        self.lead_id.write({
            'course_wise_webinar': True,
            'course_wise_webinar_dt': fields.Datetime.now()
        })
